...
1. FIELD_CONFIGS（原有）：每个字段组（deal_summary/acquisition_purpose）对应
   一套完整的system prompt + user prompt + 单一检索query。
   → 用于Round 1的LLM全字段抽取。

2. FIELD_SUBGROUPS（新增 - 多轮fallback）：将每个字段组拆分为多个SubFieldGroup，
   每组有独立的检索query、HyDE提问、章节关键词。
   → Round 1: 按子组分别检索，覆盖更广
   → Round 2: HyDE用hyde_query生成假设文本
   → Round 3: 用section_keywords定位相关章节

设计原则：
- FIELD_CONFIGS保持不变，确保向后兼容
- FIELD_SUBGROUPS是并行的增强结构，只在enable_multi_round=True时使用
"""
...
#
# 问题背景：
#   原来DEAL_SUMMARY_QUERY = "交易方案 交易金额 发行价格 支付方式 标的估值..."
#   把9个概念塞进一个query做embedding，向量"平均化"后偏向高频概念（交易金额），
#   导致低频概念（估值方法、业绩承诺）对应的chunks检索不到。
#
# 解决方案：
#   拆成4个语义聚焦的子组（parties/pricing/valuation/commitment），
#   每组有独立的检索query，Round 1分别检索后合并去重。
#   如果Round 1后某些字段仍为空，Round 2/3用该组的hyde_query和section_keywords继续找。
# =====================================================================
...
class SubFieldGroup:
    ...
    每个SubFieldGroup代表一组语义相近的字段，共享同一套检索策略。
    例如 valuation 组覆盖 target_valuation 和 valuation_method，
    它们都集中在报告的"评估与定价"章节中。

    Attributes:
        name: 组名，用于日志标识（如 "valuation"）
        query: Round 1 向量检索用的query文本，会被embed后与chunk向量计算相似度
        target_fields: 此组负责覆盖的主字段名（不含_quote后缀）
        section_keywords: Round 3 章节路由用的关键词列表，
            chunk.section中包含任一关键词即匹配（子串匹配）
        hyde_query: Round 2 HyDE用的自然语言提问，
            LLM会根据此问题生成一段假设性报告原文，再用该文本的embedding去检索
    """
...
# --- deal_summary 的4个子字段组 ---
# 拆分逻辑：按信息在报告中的物理位置分组
#   parties   → 通常在"交易概述"开头
#   pricing   → 通常在"交易方案"/"发行股份情况"
#   valuation → 通常在"评估与定价"专章
#   commitment→ 通常在"业绩承诺"/"盈利预测补偿协议"
...
DEAL_SUMMARY_SUBGROUPS = [
    ...
    SubFieldGroup(
        ...
        # 这几个关键词能覆盖：
        # "第六节 交易标的的评估与定价" / "第六章 标的资产评估及定价情况"
        # / "第六节 标的资产估值情况" 等不同报告的章节命名
...
# 字段名 → 子字段组列表（供extractor.py按field_name查找）
FIELD_SUBGROUPS = {
...
# =====================================================================
# HyDE (Hypothetical Document Embeddings) prompt模板
#
# 原理：
#   直接用"估值方法"4个字做embedding，语义太稀疏，检索效果差。
#   HyDE的思路是让LLM先生成一段"假设性的报告原文"，比如：
#   "本次交易采用收益法和资产基础法对标的公司100%股权进行评估，
#    最终以收益法评估结果作为评估结论，评估值为56,025.23万元。"
#   再把这段假设文本的embedding去检索，就能精准匹配到真正的评估章节。
#
# 关键设计：
#   - 要求用中文书面语，模拟报告原文风格（与实际文档语体一致）
#   - 要求包含虚构细节（金额、公司名），让embedding包含足够的语义信号
#   - 控制100-200字，太短语义不够，太长会稀释
# =====================================================================
...
# =====================================================================
# 定向抽取prompt模板（Round 2/3专用）
#
# 与Round 1的区别：
#   Round 1: 用完整的DEAL_SUMMARY_SYSTEM抽取所有字段（acquirer, target, ...全部）
#   Round 2/3: 只针对缺失字段（如valuation_method）进行定向抽取
#
# 这样做的好处：
#   1. 节省token——不需要重复抽取已有字段
#   2. LLM更聚焦——只关注缺失的几个字段，准确率更高
#   3. prompt中{field_descriptions}是动态构建的，只包含需要的字段描述
# =====================================================================
...
# 字段描述映射：字段名 → 定向抽取prompt中的描述文本
# 在_targeted_extract()中，根据缺失字段动态拼装到TARGETED_EXTRACT_SYSTEM中
FIELD_DESCRIPTIONS = {
    # deal_summary字段
